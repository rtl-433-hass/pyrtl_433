"""Device-library mapping package for rtl_433 field data.

A thin, dependency-light loader + query layer that parses the themed YAML files
shipped under ``library/data/`` into an in-memory registry of
:class:`FieldDescriptor` objects keyed by rtl_433 field name, loads the
``_skip_keys.yaml`` exclusion list, optionally layers a per-installation user
override on top, and resolves / transforms field values for the consumer's
entity model.

The descriptor vocabulary (``platform`` / ``device_class`` / ``state_class`` /
``entity_category`` / ``object_suffix``) is the Home Assistant MQTT-discovery
vocabulary, so a consumer maps it 1:1 onto its own entities. Nothing here
imports Home Assistant: the package is pure ``yaml`` + stdlib.
...
* ``_transform`` -- raw-value to entity-state conversion (:func:`apply_transform`).
...
File I/O is synchronous. Async callers must run :func:`load_library` off the
event loop (e.g. via :func:`asyncio.to_thread`) because it touches the
filesystem.
...
from ._model import (
    MODELS_FIELD as MODELS_FIELD,  # re-export
    SKIP_KEYS_FIELD as SKIP_KEYS_FIELD,  # re-export
...
def _default_library() -> tuple[Registry, set[str]]:
    ...
    :func:`load_library` off the event loop (e.g. via
    :func:`asyncio.to_thread`) and pass the result explicitly so the event loop
    is never blocked.
    """
...
def event_driven_field_keys(registry: Registry | None = None) -> frozenset[str]:
    ...
    eventually misfire; a consumer feeds this set to
    :func:`pyrtl_433.availability.is_event_driven` and maps the resulting boolean
    onto its own availability-timeout policy.
    ...
    library and any per-installation user mappings. ``registry`` defaults to the
    cached shipped library.
    """
    if registry is None:
