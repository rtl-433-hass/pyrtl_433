"""YAML device-library loader for the library package.

Parses the themed ``data/*.yaml`` files into a :class:`Registry` (flat table +
model-scoped overlay), loads ``_skip_keys.yaml``, and merges a per-installation
override mapping on top. Every entry point is defensive: a malformed file,
model, or entry is logged and skipped rather than aborting the whole load.
"""
...
# loader works regardless of the installed location (``.parent`` is the
# ``pyrtl_433/library`` package directory, which ships ``data/*.yaml`` as
# package data).
_LIBRARY_DIR = Path(__file__).parent / "data"
...
def _descriptor_from_entry(field_key: str, entry: dict[str, Any]) -> FieldDescriptor:
    ...
    # both mean "let the consumer auto-name from device_class".
...
def load_library(
    ...
    Globs ``<library_dir>/*.yaml`` (default: the packaged ``data``
    ...
    This performs blocking file I/O; async callers must run it off the event
    loop (e.g. via :func:`asyncio.to_thread`).
    """
