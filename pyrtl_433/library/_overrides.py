"""User-override validation and normalization for the library package.

Pure helpers (no I/O) used by a consumer's configuration surface and the loader:
validate a user-supplied mapping object into a list of self-contained problem
strings, and produce a deep-copied, JSON-serialisable, payload-canonical copy
for storage.
...
def validate_user_mappings(data: Any) -> list[str]:
    ...
    names the offending field/path) so a configuration UI can join them into a
    single form error.
    """
    if data is None:
        return []
