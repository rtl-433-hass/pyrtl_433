"""Value transformation for the library package.

Turns a raw rtl_433 value into the consumer-facing state: binary ``payload``
mapping for ``binary_sensor`` descriptors, and the ``value_transform`` pipeline
(coerce -> scale -> offset -> round) for ``sensor`` descriptors. Defensive
throughout: invalid transform parameters are logged and skipped rather than
raising.
"""
...
def _apply_factor(
    ...
    if key not in transform:
        return number
    try:
        # ``combine`` is ``operator.mul``/``operator.add``: numeric in, numeric out.
        return combine(number, float(transform[key]))  # type: ignore[no-any-return]
    except TypeError, ValueError:
...
def apply_transform(descriptor: FieldDescriptor, raw_value: Any) -> Any:
    """Convert a raw rtl_433 value into the consumer-facing state.
