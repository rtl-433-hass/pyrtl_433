#!/usr/bin/env python3
"""Deterministically partition the library's modules into N balanced shards.
...
by its measured mutmut run time from ``scripts/mutation_timings.json`` (see
scripts/mutation_timings.py). A module absent from that profile (a newly added
file, or a stale profile) falls back to ``mutant_count * avg_seconds_per_mutant``
so it is still placed sensibly. Modules are sorted heaviest first (ties by path),
then each is placed into the currently-lightest bin (ties by lowest index) — the
...
def patterns_for(paths: list[str]) -> list[str]:
    ...
    A normal module ``a/b.py`` matches ``pyrtl_433.a.b.*``.
    ...
    e.g. ``pyrtl_433.x_connect__mutmut_1``. A bare ``pyrtl_433.*`` would also match
    ...
    starts with those, so this matches exactly the package-root mutants and
    nothing else.
    """
    patterns: list[str] = []
    for p in paths:
