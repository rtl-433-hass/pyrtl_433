...
still has a slow pole. This script records each file's measured mutmut run time —
the sum of its mutants' actual test durations (``durations_by_key`` in mutmut's
per-file meta) — which ``scripts/mutation_shards.py`` then uses as the bin-pack
weight.
