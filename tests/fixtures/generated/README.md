...
`rtl_433_tests` repository. They are vendored here from the
[rtl-433-hass/rtl_433](https://github.com/rtl-433-hass/rtl_433) Home Assistant
integration, whose `scripts/regen_capture_fixtures.py` regenerates them and
whose `.github/workflows/captures.yml` re-decodes and diffs them against a
pinned image. Refresh them by copying that repository's
`tests/fixtures/generated/` over this directory, never by editing by hand.
...
asserts each field resolves to a `pyrtl_433.library` descriptor. For the
hand-authored fixtures that check is only as good as the hand-authoring; against
these, it checks the device library against what rtl_433 actually puts on the
wire.
