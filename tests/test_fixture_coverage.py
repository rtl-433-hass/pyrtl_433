...
resolves to nothing builds no entity, so a user with that device sees a sensor
that never appears — and nothing anywhere raises.
...
decoded from real ``.cu8`` captures by a pinned rtl_433 image, so the sweep runs
against rtl_433's actual wire output rather than a hand-written approximation of
it. See ``tests/fixtures/generated/README.md``.
...
def test_fixture_fields_all_resolve_to_descriptors(fixture_name, library, load_fixture):
    """Every non-skipped field in every fixture event maps to a descriptor.

    Resolution is model-scoped, matching what a consumer does when it decides
    whether to build an entity for a field -- not the model-agnostic flat lookup
    an unmapped-field report uses.
    """
    ...
    assert not unmatched, (
        ...
        "pyrtl_433/library/data/, or add the key to _skip_keys.yaml if it is "
        "identity or transport data."
    )
