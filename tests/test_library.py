...
These exercise the shipped library's mapping semantics: descriptor resolution,
...
def test_existing_themed_file_loads_identically(library, tmp_path):
    ...
    src = Path(__file__).resolve().parents[1] / "pyrtl_433" / "library" / "data"
...
# --------------------------------------------------------------------------- #
# The packaged data directory ships and is what a bare load_library() finds.   #
# --------------------------------------------------------------------------- #
def test_load_library_defaults_to_the_packaged_data_directory(library):
    """A no-argument ``load_library()`` reads ``pyrtl_433/library/data``.

    The YAML is package data, not source, so it only reaches an installed wheel
    via ``[tool.setuptools.package-data]``. If that wiring breaks, ``_LIBRARY_DIR``
    resolves to a directory that does not exist and ``load_library()`` returns an
    empty registry with no skip keys and no error — every field silently unmapped.
    This pins the default to the real packaged directory and to a full load.
    """
    from pathlib import Path

    import pyrtl_433.library
    from pyrtl_433.library._loader import _LIBRARY_DIR

    package_dir = Path(pyrtl_433.library.__file__).resolve().parent
    assert _LIBRARY_DIR.resolve() == package_dir / "data"
    assert _LIBRARY_DIR.is_dir()

    yaml_files = sorted(path.name for path in _LIBRARY_DIR.glob("*.yaml"))
    assert yaml_files == [
        "_skip_keys.yaml",
        "air_quality.yaml",
        "binary_states.yaml",
        "events.yaml",
        "humidity_moisture.yaml",
        "light_uv.yaml",
        "misc.yaml",
        "power_electrical.yaml",
        "pressure.yaml",
        "rain.yaml",
        "temperature.yaml",
        "wind.yaml",
    ...
    # Descriptor count cross-checked against an independent parse of the same
    # files, so this stays true as mappings are added but still fails loudly if
    # a file stops being read (or is read twice).
    expected_keys: set[str] = set()
    for path in _LIBRARY_DIR.glob("*.yaml"):
        if path.name.startswith("_"):
            continue
        data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        expected_keys |= set(data) - {"models"}
    assert set(registry.flat) == expected_keys
    assert len(registry.flat) == len(expected_keys) >= 60

    # The shipped skip list is identity/transport data only, and is stable.
    assert skip_keys == {
        "EndpointID",
        "PacketCRC",
        "channel",
