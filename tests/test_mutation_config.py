...
The mutation gate is driven by ``mutmut-ratchet``
(https://github.com/rtl-433-hass/mutmut-ratchet), which is tested in its own
repository. What that tooling cannot check is the per-repo data it is pointed
at: the ``[tool.mutmut_ratchet]`` table in ``pyproject.toml`` and the committed
...
* A test whose filename maps to no source module escalates *every* PR that
  touches it to a full run — slow, and easy to introduce without noticing.
* A baseline file that lands in zero shards (or in two) escapes the per-file
  floor entirely, or is double-counted.
...
def test_no_test_file_silently_escalates(config: Config) -> None:
    ...
    non-existent module silently escalates every touching PR to a full mutation
    run. A new such test fails here until it is added to the
...
def test_every_baseline_file_lands_in_exactly_one_shard(
    ...
    real committed baseline for every split width CI might use.
    """
