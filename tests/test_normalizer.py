...
Ported from tests/test_normalizer.py of rtl-433-hass/rtl_433 (Apache-2.0).
"""
...
from pyrtl_433.normalizer import device_key, normalize
...
def test_normalize_real_event_fixtures():
    """Every bundled fixture event normalizes to a stable key with time dropped.

    Exercises the copied ``fixtures/*.json`` sample frames end-to-end: each real
    rtl_433 event yields a non-empty device key, drops ``time`` from ``fields``,
    and keeps its measurement values (a couple checked exactly).
    """
    events: dict[str, dict] = {}
    for path in sorted(_FIXTURES.glob("*.json")):
        for event in json.loads(path.read_text()):
            normalized = normalize(event)
            assert normalized.device_key
            assert "time" not in normalized.fields
            events[normalized.device_key] = normalized.fields

    # Multi-identity keys resolve as expected across the fixture set.
    assert "Acurite-606TX-42" in events
    assert events["Acurite-606TX-42"]["humidity"] == 56  # second frame wins
    # A subtype-carrying lightning frame keeps its measurement fields.
    assert events["AcuriteLightning-6045M-200-st2"]["strike_count"] == 3
    # A channel-only frame keys off the channel token.
    assert "Prologue-TH-ch3" in events
